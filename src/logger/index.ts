...
export function error(context: string, ...args: any[]) {
  if (currentLogLevel <= LogLevel.debug) {
    if (currentLogType === LogType.json) {
      logger.error(`[${new Date().toISOString()}]`, context, ':  ', JSON.stringify(args, null, 2));
    } else {
...
export function debug(context: string, ...args: any[]) {
  if (currentLogLevel <= LogLevel.debug) {
    if (currentLogType === LogType.json) {
      logger.debug(`[${new Date().toISOString()}]`, context, ':  ', JSON.stringify(args, null, 2));
    } else {
...
export function info(context: string, ...args: any[]) {
  if (currentLogLevel <= LogLevel.info) {
    if (currentLogType === LogType.json) {
      logger.info(`[${new Date().toISOString()}]`, context, ':  ', JSON.stringify(args, null, 2));
    } else {
...
export function warn(context: string, ...args: any[]) {
  if (currentLogLevel <= LogLevel.info) {
    if (currentLogType === LogType.json) {
      logger.warn(`[${new Date().toISOString()}]`, context, ':  ', JSON.stringify(args, null, 2));
    } else {
